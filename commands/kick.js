...
async function kickCommand(sock, chatId, senderId, mentionedJids, message) {
    ...
    if (!isOwner) {
        ...
        if (!isBotAdmin) {
            await sock.sendMessage(chatId, { text: 'ꜱʏʀᴇɴᴛʜᴀ! Please make the bot an admin first.' }, { quoted: message });
            return;
        }

        if (!isSenderAdmin) {
            await sock.sendMessage(chatId, { text: 'ꜱʏʀᴇɴᴛʜᴀ!Only group admins can use the kick command.' }, { quoted: message });
            return;
        }
    }
    ...
    try {
        ...
        await sock.sendMessage(chatId, { 
            text: `${usernames.join(', ')} ꜱʏʀᴇɴᴛʜᴀ has kicked successfully!`,
